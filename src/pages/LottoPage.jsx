...
const ANALYSES = [
  ...
  },
  {
    ...
    resultReview: [
      {
        title: '실제 결과 분석',
        text: '1238회 실제 당첨번호는 2, 13, 18, 32, 38, 42이고 보너스는 22였다. 1등은 23게임, 1게임당 당첨금은 1,197,258,718원으로 기록했다. 무진일 분석에서는 13·18·32·42 네 숫자를 후보권 안에 넣었고, 2와 38은 추천 후보에서 빠졌다.',
      },
      {
        title: '가장 맞았던 흐름',
        text: 'CODE B가 13·18·42 세 숫자를 맞췄다. 무진일 빈도와 최근 HOT 흐름을 교차시킨 방향이 이번 결과와 가장 가까웠고, 특히 42를 고구간 방어수로 열어둔 판단은 유효했다.',
      },
      {
        title: '부분 적중 판단',
        text: 'CODE A는 13·32 두 숫자를 맞췄고, CODE C는 32, CODE D는 18을 각각 맞췄다. CODE E는 본번호는 빗나갔지만 날짜수로 넣었던 22가 보너스로 들어왔다. 전체적으로 무진일 자체 빈도만 밀기보다 최근 흐름과 섞은 조합이 더 나았다.',
      },
      {
        title: '다음 회차 보정',
        text: '이번 패착은 2처럼 낮은 이탈수와 38처럼 중후반 보정수를 충분히 열지 못한 점이다. 다음 분석에서는 중심축 2~3개를 유지하되, 1~10 구간의 단독 이탈수와 35~40 구간의 중간 방어수를 최소 하나씩 더 강하게 배치해야 한다.',
      },
    ],
    picks: [
      {
  ...
  },
  {
    ...
    aiOpinion: [
      {
        title: '주력 판단',
        text: 'CODE B는 이번 주 주력이다. 1·37·42는 을해일 최상위권이면서 최근 흐름도 살아 있고, 9·25는 을해일 4회와 최근 20회 3회가 겹친다. 18은 을해일 자체 빈도는 3회지만 최근 20회 6회로 가장 뜨거운 축이므로 과거 을해일과 현재 흐름의 교집합으로 본다.',
      },
      {
        title: '확장 판단',
        text: 'CODE C는 최상위 빈도만 고집하지 않고 현재 흐름이 밀어주는 13·15·22를 받는 확장판이다. 지난주 13·18·42처럼 일진에서는 중상위지만 최근 흐름이 강한 숫자를 놓치지 않기 위한 구성이다.',
      },
      {
        title: '보험 판단',
        text: 'CODE D는 주력 조합과 성격을 일부러 다르게 둔 잠복 복귀형이다. 16·30·33·45처럼 현재 미출현 기간이 길어진 숫자를 넣어, HOT 흐름만 보면 놓칠 수 있는 복귀수를 받는 보험 조합으로 둔다.',
      },
      {
        title: '날짜수 판단',
    ...
    picks: [
      {
        name: 'CODE A · 을해일 정통형',
        numbers: [1, 6, 16, 37, 40, 42],
        note: '을해일 20회에서 1이 6회로 단독 1위, 4·6·16·37·40·42가 모두 5회다. 그 최상위권만으로 거의 구성한 순수 을해일형 조합.',
        purchased: true,
      },
      {
        name: 'CODE B · 이번 주 주력',
        numbers: [1, 9, 18, 25, 37, 42],
        note: '지난주 3개 잡았던 B 논리를 이어간 주력 조합. 1·37·42는 을해일 최상위권이면서 최근 흐름도 살아 있고, 9·25는 을해일 4회와 최근 20회 3회가 겹친다. 18은 최근 20회 6회로 현재 가장 뜨거운 축.',
        featured: true,
        purchased: true,
      },
      {
        name: 'CODE C · 교차 확장',
        numbers: [4, 13, 15, 22, 40, 42],
        note: '일진 최상위만 고집하지 않은 B의 확장판. 13은 최근 20회 5회, 15·22·42는 각각 4회이고, 여기에 을해일 강수 4·40·42를 연결했다.',
        purchased: true,
      },
      {
        name: 'CODE D · 잠복 복귀',
        numbers: [7, 16, 30, 33, 43, 45],
        note: '16은 을해일 5회이면서 현재 11회 미출현, 30은 10회 미출현, 33은 15회 미출현, 45는 14회 미출현이다. 현재 HOT만 보면 못 잡는 복귀수를 노리는 보험 조합.',
        purchased: true,
      },
      {
        name: 'CODE E · 8월 29일 미신형',
        numbers: [8, 16, 21, 29, 37, 43],
        note: '8월의 8, 29일의 29, 8+29의 37, 29-8의 21을 반영했다. 37은 을해일 5회 출현 강수라 날짜수와 데이터가 겹치는 핵심이고, 16·43을 보강했다.',
        purchased: true,
      },
      {
        name: 'CODE F · 역발상 조합',
