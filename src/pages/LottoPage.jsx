...
const ANALYSES = [
  ...
  },
  {
    ...
    aiOpinion: [
      ...
      {
        title: 'AI 최종 의견',
        text: '원문 기준 정미일 추천은 26을 모든 조합의 고정축으로 둔 다중 패턴 포트폴리오다. 여기에 추가 14번은 패착 보정 v2를 적용해 중심축 26, 정미일 보조축 4·34, 경자일 잔향 15, 변칙 방어수 31·43을 함께 본 종합형으로 둔다.',
      },
    ],
    resultReview: [],
    picks: [
      ...
      {
        name: '14 · 패착 보정 종합형',
        numbers: [4, 15, 26, 31, 34, 43],
        note: '중심축 26, 정미일 보조축 4·34, 경자일 A주차 잔향 15, 변칙 방어수 31·43을 섞은 추가 종합 조합',
        featured: true,
      },
    ],
  },
...
const METHOD_GROUPS = [
  ...
  },
  {
    title: '4. 패착 보정',
    items: [
      '패착 보정은 빠진 번호를 다음에 그냥 추가하는 방식이 아니라, 빠진 번호가 후보로 올라오지 못한 분석 구조를 고치는 작업이다.',
      '경자일에서는 15·25가 과하게 반복됐고, 31·43 같은 변칙 방어수가 후보 역할을 받지 못했다.',
      '다음 분석부터 번호를 중심축, 보조축, 변칙 방어수로 나누고, 중심축은 강하게 두되 보조축 반복은 제한한다.',
      '변칙 방어수는 빈도 최상위가 아니어도 구간을 열어주거나 용지 분산을 보완하거나 인접 주차 기운을 이어주면 별도 점수를 준다.',
      '정미일 14번 조합은 이 보정 원칙을 적용해 26을 중심축으로 두고, 4·34 보조축과 15·31·43 방어수를 함께 배치했다.',
    ],
  },
  {
    title: '5. 용지 분산 점수',
  ...
  },
  {
    ...
    items: [
      ...
      'A주차는 B로 들어오기 전의 잔향, C주차는 B 다음으로 빠져나가는 방향으로 보고, B주차 숫자 해석의 배경값으로 둔다.',
